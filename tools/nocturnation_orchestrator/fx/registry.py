...
``@fx_registry.register`` class decorator. The runner uses ``get()`` to
look up an incoming cue's ``fx_id`` and instantiate the matching class.
...
Id allocation policy: 1..254. Id 0 is the cancel sentinel (``stop`` cue
in the YAML); id 255 is reserved. Categories carve out ranges by
convention only - the registry does not enforce them:
...
class FxRegistry:
    ...
    Effects register via the ``@register`` class decorator. Lookup
    returns None for unknown ids; the runner silently drops unknown
    ids so YAML cue lists can reference future-only effects without
    breaking older orchestrator builds.
    """
    ...
    def all_ids(self):
        """Sorted list of registered fx_ids. Used by the FX library
        doc generator (B4) and diagnostics."""
