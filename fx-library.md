...
### Group Drift Wash With Sparkle (id 16)

`group_drift_wash_with_sparkle` - beat

Phase-offset drift wash across groups 1..num_groups plus a cascading sparkle one group per beat. On a static display with groups physically in order this reads as a moving colour wave. Each group's wash is emitted once at its scheduled phase offset; the Lume drifts locally, so wire traffic stays low.
...
| 9    | s_b         | u8      | Sparkle Blue.                                        |
| 10   | probability | percent | Sparkle chance per beat (0..100%). Default 100%.     |
| 11   | num_groups  | count   | Groups to cascade across (1..9). Default 4.          |
| 12   | attack      | u8      | Sparkle attack slider 0..255. Default 16 when zero.  |
| 13   | sustain     | u8      | Sparkle sustain slider 0..255. Default 16 when zero. |
| 14   | release     | u8      | Sparkle release slider 0..255. Default 96 when zero. |
