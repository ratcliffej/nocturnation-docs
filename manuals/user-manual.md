...
**Static repeat mode (StickC, mesh extension).** A Stick-form Lume with `Connectivity > ESP-NOW > Lume Repeat = ON` rebroadcasts every accepted frame, with the frame's `hop_count` incremented by one. The frame's `source_id` and `sequence_number` are preserved so the fleet-wide dedup still works mesh-wide. The hop ceiling is **3** — frames beyond that are silently dropped as loop prevention. Use a repeater to bridge a corner or extend coverage past a wall: place the repeater with line of sight to both the Director and the audience-side Lumes. The repeater Stick shows a small `R:N` counter at the bottom-left of its LCD when Lume Repeat is on, where N is the total number of frames it has rebroadcast since entering Lume mode — useful for confirming the relay path is firing during a deployment.

**Dynamic repeat mode (Tildagon, opportunistic mesh).** Tildagon-form Lumes support a self-organising variant of the same idea. With `Config > Repeat = AUTO` (the default), a Tildagon that finds itself at an **unserviced edge of the mesh** — receiving signal at one hop level with no relay pushing it further out — silently elects itself as a repeater. When a peer takes over, or the badge finds itself back in the coverage core, it steps back down. The rule is: a badge elects only when it can't hear the same frame at a lower hop. A Tildagon that receives a frame at both hop 0 (direct from Director) and hop 1 (via another repeater) is in the coverage core and stays LISTEN; a Tildagon that only hears hop 1 (out of Director's direct range) is at an edge and elects to relay at hop 2. Use dynamic repeat in open-air venues where fixed repeater placement isn't possible: an audience of Tildagons becomes an opportunistic mesh that self-organises around coverage gaps as the crowd moves. Recovery from a walked-away repeater takes 3-9 seconds. The two repeat variants coexist without conflict — a StickC static repeater's continuous hop 1 output naturally suppresses audience-Tildagon shell 1 election in its coverage core, and badges further out cascade to shell 2. The Tildagon LCD shows the FSM state (LISTEN, LISTEN?, REPEAT, CDOWN) on the standard HUD; enable `Config > Debug = ON` for a prominent state readout on the debug overlay with `tx:` and `px:` counters. Full description in the [Tildagon app README](https://github.com/ratcliffej/nocturnation-tildagon#repeat-mode).
