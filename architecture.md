...
- **Per-beat punctuation** is layered on top, but gated. Fire on selected beats - downbeats, snare-emphasis beats, beats during the chorus - not every beat. NocturNation's `chance` parameter on `LIGHT_COMMAND` was designed for exactly this gating: a Show can declare "flash on 25% of detected beats" and the protocol will distribute the firings probabilistically across Lumes.
