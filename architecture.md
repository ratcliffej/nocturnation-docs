...
# NocturNation Architecture Specification

**Status:** Draft v0.32 - early architecture document, expect substantial revision.
...
Original prototyping work: [Original prototype](https://www.notion.so/358bd067740580bab876cd7c2b7ee6bf) 
### Design principles
...
---
## 1.2 Lighting design principles
NocturNation can technically flash a Lume on every kick drum. Working concert lighting designers - the people who actually do this for a living - almost never do that, and the reasons are worth being explicit about because they should shape how Shows are written, how Director Mode behaves on Tildagon at EMF, and what the project tells contributors a "good" show looks like.
This section is normative for Show authors. The protocol is permissive - it will carry whatever the Director sends - but the project's design aesthetic is restraint. Showing this understanding is also part of how NocturNation earns credibility with the EMF community and any future professional partners (see Epic 6 launch criteria).
### Three timescales, only one of which is the beat
Professional lighting design operates on three layered timescales, ordered from longest to shortest:
**The song arc (minutes).** Palette and overall intensity shift between sections of a song. Verse is restrained, chorus opens up, drop is the moment of maximum visual energy, breakdown drops everything to near-darkness so the next chorus can land. This is where the creative decisions happen. Felix Peralta, the Emmy-winning concert LD for Café Tacvba and others, frames this as "reflecting timing, pacing, and tempo" - the lighting tracks the song's emotional contour, not its surface rhythm (Chauvet, 2024).
**The musical phrase (8-32 beats / a few seconds).** Movement, sweeps, and washes that build or release tension across bars. A slow upward intensity sweep across a chorus. A colour drift across a bridge. A gradual saturation increase as a drop approaches. These are the gestures that make a show *feel composed* rather than reactive.
**The beat (sub-second).** Punctuation. Used sparingly to accent moments, not to fill every space. Professional shows might use per-beat flashing 10-20% of the time, and even then on selected beats - the "one" of each bar, drops, snare hits on emphasis, the last beat before a section change.
The industry framing is consistent on this: "Build a few base lighting looks that match the mood and pace of each song type" (SHEHDS, 2025) - foundational layers that hold for whole sections - "Not everything needs equal brightness simultaneously" (ibid). The beat is a layer *on top* of the slower decisions, not a replacement for them.
NocturNation's analyser already produces all three primitives: section detection (§5.1 step 9), drop / breakdown events, beat detection. The architecture is already pointed at the right surfaces. What's been missing in earlier Show drafts is the orchestration philosophy that uses them in proportion.
### Bass as the lead, not hi-hat
When lighting *does* respond to discrete rhythmic events, the bass region (kick drum, sub-bass) is almost always the right driver. Two honest reasons:
Bass is what the audience physically *feels*. Kick drums and bass lines hit listeners in the chest. Lighting synchronised to bass synchronises with the part of the music already in the audience's body, which makes the visual coupling feel emotionally connected rather than mechanically clever.
Bass changes slowly enough to look intentional. Typical 4/4 music at 120 BPM has 2 kicks per second; the same music has 4 snares per second and 8-16 hi-hats per second. Driving lights from kick gives 2 punctuations per second - musical. Driving from hi-hat gives 8-16 - exhausting. NocturNation's existing sub-band detection separates these (§5.1 step 6); the design guidance is to *prefer the lower bands* as primary beat drivers and to use higher-band onsets sparingly, if at all.
Felix Peralta, a former drummer, captures this directly: "musicality and rhythm must find a way to run through the heart and soul of every memorable lighting design" - his approach is to "go beyond understanding the mood of the music, or the meaning of its lyrics... striving to reflect its timing, pacing, and tempo" (Chauvet, 2024). The bass region is where timing, pacing, and tempo are most legibly visible across genres.
### Restraint earns the moments that matter
The single most important insight from professional practice is this: visual impact comes from *contrast*, and contrast requires restraint elsewhere. A drop only feels like a drop if the verse before it was quieter. A strobe only lands if the song didn't strobe through every bar leading up to it. A whiteout only shocks if the palette wasn't already dominated by white.
Chris Lose, host of the LD-at-Large podcast and a working concert LD, has interviewed Music-by-Sia lighting designers explicitly on the "less-is-more principle" (Lose, 2019-present). The framing applies across genres: shows that look effortless and powerful are almost always shows where the LD chose *not* to flash on many beats.
This maps to a concrete Show design pattern for NocturNation:
- **Continuous wash (slow-changing background)** is the default. The analyser's centroid and energy descriptors (§5.1 step 8) drive a slow hue / brightness contour that holds across most of a song.
- **Per-beat punctuation** is layered on top, but gated. Fire on selected beats - downbeats, snare-emphasis beats, beats during the chorus - not every beat. NocturNation's `chance` parameter on `LIGHT_PULSE` was designed for exactly this gating: a Show can declare "flash on 25% of detected beats" and the protocol will distribute the firings probabilistically across Lumes.
- **Section transitions** are the bigger moments - palette shifts, brightness changes, the lights *responding to the song's structure* rather than its surface beat. The section detector (§5.1 step 9) emits these as state changes.
- **Drops** are the climactic gestures. They should look different from everything around them - which means everything around them needs to look restrained.
### Hard floors from safety, not just taste
The restraint argument is mostly aesthetic but there's a medical floor underneath it. Flashing lights between 3 and 60 Hz can trigger photosensitive epilepsy seizures in susceptible audience members, with peak sensitivity at 15-25 Hz (Fisher et al., 2022). Even well below seizure threshold, sustained high-frequency flashing causes visual fatigue and reduces the perceptual impact of any individual flash.
NocturNation's hard 4 Hz frequency cap (§15.1) keeps the system below the most dangerous range, and Calm Mode (§15.3) drops the cap further to 2 Hz. But the safety floor is *much* lower than where a good Show should be operating most of the time. A Show that's running per-beat flashes for the entire duration of a song at 120 BPM is operating at 2 Hz constantly - within Calm Mode's cap and within NocturNation's 4 Hz hard cap, but well into the range that's both medically meaningful for sensitive viewers and aesthetically tiring for everyone else. The aesthetic guidance (restraint, layered timescales, bass-led punctuation) tends to keep effective flash rates well below the safety floor as a side-effect of being designed well. This is not coincidence; the LD craft accumulated these patterns partly because audiences self-select away from shows that exhaust them.
### Implications for Show authors
The practical guidance distils to a checklist Show authors can hold themselves to:
Does your Show consume `section` and the drop / breakdown events, or does it only consume per-beat? If only per-beat, it's likely too uniform across a song. The section detector exists precisely so Shows can differentiate verse from chorus from drop.
Does your Show fire on *every* detected beat, or does it gate firings via `chance`, section state, or beat-position-in-bar? Defaulting to firing on every beat is the most common Show-authoring mistake and the one most likely to look amateur.
Is there a continuous baseline (Background Wash, slow hue drift, centroid-driven colour contour) that holds across the song? Pulses landing against a baseline read as punctuation; pulses landing on black read as flashing.
When the Show fires its biggest gesture (the drop, the section-change whiteout, the strobe burst if Strobe is enabled), is the moment *before* it visibly quieter? The drop only lands if the build-up is restrained.
Does the Show treat bass-band onsets differently from snare and hi-hat onsets? The default should be bass-led with higher bands as secondary or off entirely. Shows that fire identically on all three onset bands are almost always too busy.
### Implications for Director Mode on Tildagon
For the EMF 2026 app submission specifically (Epic 6 Workstream B), Director Mode on Tildagon will give attendees a button-driven and IMU-tap-driven show. The temptation is to make every input fire a dramatic effect because that's what feels responsive at the per-button level. The honest design choice is to layer:
- **Tap-to-beat (IMU)** produces the per-beat punctuation, gated by `chance` so not every tap fires every Lume. This gives the audience-experience of "tapping together drives the room" without the show being exhausting if 30 attendees all tap at once.
- **A held primary button (Btn-A long-press)** triggers a manual *section gesture* - palette shift, brightness change, drop-style whiteout. Used sparingly by the user, this becomes the equivalent of a lighting director hitting a section change.
- **The background is always-on**, drawing on a slow hue contour even when no taps or buttons fire. The badge is never visually silent; it just isn't always flashing.
This composition gives Tildagon-as-Director a defensible aesthetic position for the app store description (a real lighting designer's lighting design language, hand-driven) without claiming more than the implementation actually does.
### Implications for the DMX direction (Epic 7)
The long-term direction (§10.3) hands lighting decisions to a lighting designer driving QLC+ or another DMX console. This is the right answer for venue performances precisely because *the LD already knows all of this*. The job of NocturNation in DMX mode is to be a well-behaved fixture: respond accurately to channel commands, stay within safety floors regardless of console input, expose enough parameters that the LD has expressive range. The aesthetic decisions move out of NocturNation's Show code and into the LD's cue stack, where they belong for any performance more substantial than a hackspace gig.
This is not a retreat; it's the recognition that *NocturNation is a fixture, not a lighting designer*. The Show framework in autonomous mode is for deployments where there's no LD - hackspaces, art installations, home use, the Coldplay tribute act. The principles in this section describe what *good autonomous* looks like; DMX mode is what *good directed* looks like, and the LD brings their own version of these principles.

### Implementation differs by Lume capability (Epic 6C)

The principles above assume a Lume can *hold a baseline* — a continuous wash that the per-beat punctuation lands on top of. Epic 6C makes that capability explicit in the protocol surface and in the `BindingCapabilities` struct on each `OutputBinding` (§7.6). The §1.2 aesthetic is achievable on every Lume class, but the *implementation path* depends on whether the Lume can hold state between commands:

- **Capable Lumes** — Tildagon badges (perimeter ring + LCD) and StickC `LocalDisplayBinding` (LCD as screen-class light surface) declare `{can_pulse: true, can_wash: true, can_overlay: true}`. The Director sets the baseline with one `LIGHT_WASH` and lets each Lume run its own cosine-eased drift between the two specified colours; per-beat punctuation arrives as `LIGHT_PULSE` and the Lume blends it additively over the live wash. A new `LIGHT_WASH` to the same target supersedes (smooth palette transition); `LIGHT_WASH_END` cancels with an operator-supplied release time.
- **Pulse-only Lumes** — PixMob bracelets via `PixMobIrBinding` declare `{can_pulse: true, can_wash: false, can_overlay: false}`. The bracelet hardware is fire-and-forget IR — it cannot hold a colour between commands. The dispatch layer (§7.6) silently drops `LIGHT_WASH` / `LIGHT_WASH_END` / `LIGHT_WASH_PULSE` frames on these bindings; the Director need not know. To still serve the §1.2 baseline aesthetic on PixMob fleets, a Show that wants a continuous-wash feel can simulate one by re-broadcasting low-intensity `LIGHT_PULSE` frames at low cadence (every few seconds) on top of any beat-driven punctuation. This is a Show-design choice, not a protocol feature; the Show framework does not generate the simulation automatically.

The architectural commitment: the Director never branches on "what kind of Lume is out there" — it sends what the Show asks for, the dispatch layer routes by capability, incapable Lumes drop silently. Show authors who want their work to look right on a mixed fleet can either (a) stay pulse-only (works everywhere; the design's restraint-via-`chance`-gating is enough for many shows) or (b) layer a wash baseline knowing that pulse-only Lumes will see only the pulses — which still satisfies §1.2 as long as the pulses themselves are restrained.

### Resources
For Show authors and contributors who want to develop their lighting design instincts:
- The **LD-at-Large podcast** (Lose, 2019-present) interviews working concert LDs about philosophy and practice. The Music-by-Sia episode on "the less-is-more principle" is particularly relevant to NocturNation's design ethos.
- **Chauvet Professional's Lyrical Light interview series** publishes case studies with working LDs, including Felix Peralta's musicality-led approach (Chauvet, 2024).
- **Performance Lighting Design: How to Light for the Stage, Concerts and Live Events** (Pilbrow, 2017) is the standard reference textbook covering cue structure, lighting score notation, and the LD-as-musician framing.
- **SHEHDS' Concert Stage Lighting guides** (SHEHDS, 2025-2026) are aimed at small-venue operators and capture the genre-specific aesthetic guidance (rock, electronic, jazz) in plain-English form.
---
...
| Platform | Role | Notes |
|---|---|---|
| **M5StickS3** | Preferred reference platform; Director, IR Tx + Rx node, ESP-NOW peer | ESP32-S3-PICO-1-N8R8 (Xtensa LX7). Built-in IR LED (GPIO 46) and IR receiver (GPIO 42), ES8311 audio codec + MEMS mic, 1.14" ST7789P3 screen, 2 buttons + PMIC-managed power, BMI270 IMU, 8 MB PSRAM, native USB-OTG, BLE 5.0. Project's future reference platform now that the Plus2 is EOL. Audio analyser declares `beat_detection`, `drop_detection`, `spectrum_frame`, `band_summary` (Epic 4.5); declares operating points `[(16000, 512), (32000, 1024), (48000, 1024), (48000, 2048)]` of which only the canonical default is implemented in v0.22; HW-accelerated FFT via esp-dsp. |
| **M5StickC Plus2** (legacy) | Solo Director, IR transmitter node | ESP32-PICO-V3-02 (Xtensa LX6). Built-in IR LED (GPIO 19), with **optional second IR emitter on the GPIO 26 header pin** (e.g. an M5Stack IR Transmitter unit) for boosted coverage - both fire on each command when enabled. PDM mic via I2S, screen, 2 buttons + AXP192-managed power, MPU6886 IMU, BLE 4.2. Manufacturer EOL; supported as legacy for existing deployments. Same firmware codebase via the HAL backend split. Audio analyser declares the same feature set as the S3 (`beat_detection`, `drop_detection`, `spectrum_frame`, `band_summary`); declares one operating point `[(16000, 512)]` (codec-limited); ANSI-fallback FFT via arduinoFFT. |
| **PixMob Aurora-class bracelets** | Wearable Lumes | IR-receive, 7 RGB LEDs, 2 × AAA. Multiple model variants. |
| **EMF Tildagon badge** | Lume (audience receiver) **and** IMU tap-to-beat Director | ESP32-C3 with WiFi/BLE, round colour LCD, six perimeter buttons, twelve addressable RGB perimeter LEDs (indexed 1..12), six hexpansion connectors, IMU. MicroPython runtime. Already deployed to thousands of attendees. **No microphone**, so it cannot run autonomous *audio* analysis - but Epic 6B added an **IMU tap-to-beat Director** (the operator taps the badge; accelerometer onset detection fires the beat hook and broadcasts `LIGHT_PULSE` on channel 1), so it is no longer Lume-only. Runs the same `Show` plug-in framework as the M5 firmware, ported to MicroPython with identical hook names + capability vocabulary. WiFi and ESP-NOW share one radio, so the app drops WiFi (`wifi.stop()`) only while a Lume/Director session is actively running. See §8.9. |

### 3.2 Planned

| Platform | Role | Notes |
|---|---|---|
| **Mac/PC laptop** | Orchestration Director, console host | Runs QLC+ or similar. Bridges to embedded layer via USB-serial-to-ESP32/DMX or Ethernet-to-Art-Net. |
| **M5Stack Atom Lite** | Compact transmitter node | £12, sugar-cube-sized. Same ESP32 family as StickC. Ideal for distributed IR transmitter nodes. |
...
| Carrier | Use case | Latency | Range | Comments |
|---|---|---|---|---|
| IR (940nm, 38kHz modulated) | Director → bracelet | \<1 ms | 1-15m depending on transmitter power; line-of-sight | Per-Stick range can be doubled by adding a second emitter: the Plus2 firmware supports an optional external IR transmitter on the GPIO 26 header pin (e.g. an M5Stack IR Transmitter unit), driven alongside the built-in LED. PixMob themselves use commercial-grade IR transmitters for stadiums to great effect. |
| ESP-NOW (2.4 GHz) | Director ↔ peer nodes | 5-30 ms typical | 30-150m line-of-sight | A great option that doesn't rely on ESP32 lumes to be configured. |
| USB-CDC serial | Laptop → embedded Director | 1-5 ms | Cable length | Used to recieve DMX commands from a lighting console, e.g. QLC+ |
| Ethernet (UDP) | Console → Director, Director → Director | 1-3 ms | Cable / switched network | Used to connect relays to that in turn transmit ESP_Now or IR. |
| WiFi (UDP) | Console → Director where wired isn't practical | 5-50 ms | Building scale | Used to connect relays to that in turn transmit ESP_Now or IR. Fallback due to risks of WiFi interference. |
| Bluetooth LE | Phone-app → host (control plane); future Epic | 10-100 ms | 5-30m typical | Planned to allow a consumer app on a mobile device to interact with the lume. |
| Sub-GHz RF (LoRa) *future* | Long-range outdoor | 50-200 ms | 500m+ | A possible future option, but the latency might prove to be too high. |
...
| Protocol | Layer | Carrier | Use |
|---|---|---|---|
| PixMob CommandSingleColorExt | Light driver | IR | Single bracelet/group: RGB + ASR + chance + group restriction |
| PixMob CommandSetGroupId | Light driver | IR | One-time bracelet group assignment |
| Custom ESP-NOW frame | Event sync | ESP-NOW | Beat events, mode changes, clock sync between peers |
| DMX over Enttec Pro framing | Console input | USB-CDC serial | QLC+ / similar console controlling Director |
| Art-Net (ArtDmx) | Console input | UDP / Ethernet / WiFi | Same role as serial DMX, networked |

### 4.3 Custom ESP-NOW frame format (v1)

**Current wire is v4.** The block below is the v1 design snapshot preserved for historical rationale. For the current authoritative wire spec see [`manuals/protocol-manual.md`](manuals/protocol-manual.md) §3. v1 → v2 added the two-byte magic prefix + bumped `protocol_version` to 0x02. v2 → v3 (2026-07-25) widened `source_id` and every payload's `target_group` from u8 to LE u16 to buy futureproofing headroom for stadium-scale addressing (target_group) and birthday-paradox safety for personal-rig Directors (source_id). v3 → v4 (2026-08-20) added a three-byte `led_mode` / `led_modifier1` / `led_modifier2` trailer to `LIGHT_PULSE`, `LIGHT_WASH`, `LIGHT_WASH_END` and `LIGHT_WASH_PULSE` for parametric per-pixel addressing on Lumes with multiple addressable LEDs (Epic 18). NVS storage and config UI stay at u8 for group/source addressing; per-Lume chain topology is Lume-local (Config-mode UI on hosts with a screen, build-flag constants on headless hosts).
...
### Message types (v1)
The protocol carries five active message types as of v0.32: a presence beacon, a transient punctuation primitive, and a three-message wash family for persistent baselines on capable Lumes. Analysis lives Director-internal; the wire carries operational signals (heartbeat) and rendering decisions (the LIGHT_* family). This is a deliberate principle, not an accident - if a Lume can't make an artistic decision, the wire doesn't need to carry the data that would enable one.

| Type | Name | Payload |
|---|---|---|
| 0x00 | HEARTBEAT | 9 bytes: `tick: u32` (monotonic Director clock tick for ASR envelope alignment) • `days_since_2026: u16`  • `centiseconds_today: u24` (both little-endian; for Tier 3 wall-clock anchor + replay protection). Sent at 1 Hz, suppressed when other traffic implicitly proves the Director is alive. Tier 0/1/2 Lumes consume only the tick field; Tier 3 Lumes additionally use the date fields to validate signed frames' freshness against a persisted high-water mark. A single received heartbeat is sufficient to anchor a Tier 3 Lume's replay-protection clock. Replaces the v0.26-era HEARTBEAT (empty), CLOCK_SYNC (0x04), and TIME_SYNC (0x05) which are now folded in. See §4.4 reliability strategy. |
| 0x03 | LIGHT_PULSE | 9 bytes: `target_class: u8` (0=all classes, 0x01=light, 0x02=screen, 0x03=multi-LED+screen, 0x04+ reserved; see §7.6 plug-in surfaces for the `OutputBinding::device_class()` taxonomy), `target_group: u8` (0=all groups, 1-255 specific), `r: u8`, `g: u8`, `b: u8`, `attack: u8` (`pulse::Time` index), `sustain: u8` (`pulse::Time` index), `release: u8` (`pulse::Time` index), `chance: u8` (`pulse::Chance` index). Renamed from `LIGHT_COMMAND` in Epic 6C Phase C (wire byte 0x03 unchanged, payload unchanged); the rename frees the "LIGHT_" prefix for the wash family below. Lumes filter inbound LIGHT_PULSE on `(target_class, target_group)` against each active `OutputBinding`'s `(device_class(), configured group)` pair and only render where both axes match (`0x00` matches anything on either axis). |
| 0x06 | LIGHT_WASH | 16 bytes: `target_class: u8` • `target_group: u8` • `r1, g1, b1: u8` (start colour) • `r2, g2, b2: u8` (end colour) • `attack: u8` (100 ms units; 0-25.5 s) • `release: u8` (100 ms units; default fade-out) • `intensity: u8` (0-255 brightness scalar) • `cycle_ms: u16 LE` (one full A↔B↔A oscillation; 0 = hold r1g1b1) • `ttl_seconds: u16 LE` (0 = infinite) • `pulse_response: u8` (0 = drop PULSE while washing; 1 = accept PULSE as additive overlay). Persistent background wash on capable Lumes; cosine-eased ping-pong drift between r1/r2 over `cycle_ms`. Capable Lumes (`can_wash: true` per §7.6) render; pulse-only Lumes silently drop. See `lume-capabilities-design.md` for the full renderer contract and Epic 6C Phase D for the wire-format implementation. Reclaims the v0.27-deprecated MUSIC_EVENT slot. |
| 0x07 | LIGHT_WASH_END | 3 bytes: `target_class: u8` • `target_group: u8` • `release_time: u8` (100 ms units). Explicit cancel of an active wash, with operator-supplied fade-to-black duration that overrides the wash's own `release` field. Capable Lumes fade from the instantaneous wash colour to black over `release_time`, then exit wash mode. Capable Lumes with no active wash silently drop; pulse-only Lumes silently drop. Reclaims a previously-unassigned wire slot. |
| 0x08 | LIGHT_WASH_PULSE | 9 bytes: identical layout to LIGHT_PULSE. Dispatch semantics differ - fires only on Lumes that are *currently* in wash state; non-washing Lumes silently drop. The separate message type keeps each command's intent legible (LIGHT_PULSE: "fire on any Lume in the target group regardless of state"; LIGHT_WASH_PULSE: "fire only on the washing subset"). Reclaims a previously-unassigned wire slot. |
| 0x01, 0x02, 0x04, 0x05 | RESERVED | Removed in v0.27 protocol trim (see "Messages removed from earlier drafts" below). MUST NOT be reused. Receivers that don't recognise the type byte silently discard the frame - forward-compatible. |
| 0x09-0xFE | UNASSIGNED | Available for future Director-to-Lume commands (e.g. screen text commands, audio metadata, vendor-specific extensions). Same silent-discard rule as RESERVED. |
| 0xFF | EXTENSION | Reserved for v2 protocol extensions; ignored by v1 receivers. |

### Why five messages, not two
The original v0.27/v0.29 design rationale held that HEARTBEAT and LIGHT_COMMAND together did all the wire-side work the design needed. Epic 6C added three more types - `LIGHT_WASH`, `LIGHT_WASH_END`, `LIGHT_WASH_PULSE` - because the §1.2 lighting-design principles call for a continuous baseline that the protocol's transient-pulse model couldn't express. The minimalism principle still applies; the floor has just moved.

**HEARTBEAT cannot be folded into LIGHT_PULSE.** A Lume needs to know the Director is alive *during silence* - the quiet moments between songs, the breakdown sections where no kick fires for several seconds, the period after a Lume powers on but before any music starts. LIGHT_PULSE traffic alone leaves silent intervals ambiguous: is the Director gone, or is the music just quiet? More importantly, HEARTBEAT is what lets a Lume **lock onto a channel in advance of a show starting** - the dual-channel scan in §4.5 depends on hearing heartbeats on channel 11 vs channel 1 to make the lock decision before any LIGHT_PULSE traffic exists. Without HEARTBEAT, the channel scan would have to wait for music to start, which is the wrong UX (a Lume should be ready before the first beat, not catching up to it).

**LIGHT_PULSE cannot be folded into HEARTBEAT.** A music-reactive show fires several LIGHT_PULSE frames per second - per-beat, per-snare, per-section-transition. Forcing those into a heartbeat-rate envelope would either drop frames (visible beat misses) or push heartbeat rate to FFT rate (defeating the airtime savings the 1 Hz heartbeat exists for).

**LIGHT_WASH cannot be folded into LIGHT_PULSE.** A pulse is a transient ASR envelope on a single colour: attack, sustain, release, done. A wash is a persistent baseline that Lumes hold *between* commands, with optional drift between two colours. The pulse's design (one-shot, fades to black) cannot represent "hold this baseline" without either (a) tying up airtime with continuous re-broadcasts at every cosine-step, or (b) extending the pulse with cycle / hold fields that would make the common single-flash case carry six unused bytes. The wash deserves its own type.

**LIGHT_WASH_END cannot be folded into LIGHT_WASH.** A wash can end in two semantically-different ways: TTL expiry (the wash's own `release` field controls the fade) and explicit operator cancel (LD wants a snappier fade than the wash's default). Trying to overload LIGHT_WASH for both - sending "LIGHT_WASH with cycle_ms=0 and intensity=0" as cancel - would be ambiguous with "hold-r1g1b1 at brightness 0" (a valid solid-black wash). LIGHT_WASH_END's 3-byte payload makes the cancel explicit and lets the LD pick the fade-out independently of the wash's defaults.

**LIGHT_WASH_PULSE cannot be folded into LIGHT_PULSE.** A pulse-on-wash overlay differs from a stand-alone pulse in *who responds*: LIGHT_PULSE fires on every Lume in the target group regardless of wash state; LIGHT_WASH_PULSE fires only on the washing subset. Trying to overload LIGHT_PULSE for both would conflate two orthogonal addressing dimensions (target_class+group AND wash-state-filter) into one message. The cost of a separate type is one wire byte definition; the benefit is unambiguous semantics for the LD ("hit the *washing* Lumes specifically without touching the pulse-only ones").

The five messages occupy genuinely separate roles: HEARTBEAT is the *ambient presence channel*; LIGHT_PULSE is the *transient render decision channel*; the wash family is the *persistent baseline channel* (set, cancel, overlay). If a future addition can't slot into one of these roles, that's a signal it might be the right time to add a sixth message type - but the bar is high and the existing cases need to fail first.

**What this means for future extensions.** Likely future Director-to-Lume commands (screen text, audio metadata for a Lume showing BPM, vendor-specific extensions) would each be a new message type in the unassigned 0x09-0xFE range. They don't try to fit into the existing five because their semantics are genuinely different. The bar for adding one is that a Lume must do something *visibly different* with it than the existing types already allow - a SCREEN_TEXT carrying a string passes (no existing type can carry text); a SHOW_PALETTE_CHANGED that merely names the active palette fails (the next LIGHT_PULSE / LIGHT_WASH already encodes the colour). The unassigned range is wide enough that the protocol can extend for years without v2 churn.
...
- **BEAT_DETECTED (0x01)** - was reserved for BPM/strength metadata. No Lume-side consumer existed. If a future need arises (e.g. a Lume showing BPM on its screen), the Director can fire a custom LIGHT_PULSE with that data rather than introducing a new message type.
- **MODE_CHANGE (0x02)** - was for the Director announcing mode transitions (Test, Config, Show switches, Idle). Removed in v0.28 after honest review of every transition case: each one is implicit in the traffic pattern. Test mode = canned LIGHT_PULSE sequences arrive. Config / Idle = traffic stops, Lumes timeout to idle effect. Show switch = LIGHT_PULSE pattern changes; Lumes render whatever arrives. There is no transition a Lume needs to know semantically.
...
- **MUSIC_EVENT (0x06)** - was for DROP / BREAKDOWN / BUILD events; the Director encoded each into LIGHT_PULSE instead. **The slot was reclaimed by LIGHT_WASH in Epic 6C Phase D.**
- **SNARE_DETECTED (0x07), HIHAT_DETECTED (0x08)** - both proposed for Epic 4.7; both violated the wire-carries-only-render-commands principle. **Both slots reclaimed by the wash family in Epic 6C Phase D** (LIGHT_WASH_END = 0x07, LIGHT_WASH_PULSE = 0x08). Snare and hi-hat onsets remain Director-internal events that produce LIGHT_PULSE frames with the per-drum colour the Show decides.
- **MUSIC_DESCRIPTOR (0x09), SECTION_CHANGE (0x0A)** - same Epic-4.7 origin; same Director-internal disposition. Slots remain unassigned and available for future protocol additions.
...
- **Existing fields can carry the load.** The `source_id` field (already in every frame header) provides per-Director discrimination. Lumes can be configured to listen only to a specific source. The `target_group` field (in LIGHT_PULSE payload) provides per-device-collection discrimination. Show A using groups 1-15 and Show B using groups 16-31 is a perfectly serviceable workaround.
...
Group ID is a first-class concept in the NocturNation protocol, not a PixMob-IR-specific feature. Every Lume that understands LIGHT_PULSE has an assigned group ID, persisted across reboots. The semantics:
...
| Name | Trigger | Description | Parameters | Status |
|---|---|---|---|---|
| **Pulse** | Per-beat | Single colour fired with ASR envelope on each detected beat. The current default beat-reactive behaviour. | colour, attack, sustain, release, target group | ✅ Implemented |
| **Probability Pulse** | Per-beat | Pulse with per-target chance gating. Each receiver rolls independently and either fires or stays dark. Across multiple bracelets produces a "popcorn" twinkle effect. | colour, ASR, chance (4-100%), target group | ✅ Implemented |
| **Random Palette Pulse** | Per-beat | Pulse with colour drawn from a small palette (typically 4-8 colours) at random per beat. All targets show the same colour but it changes between beats. | palette, ASR, target group | ⏳ Designed, not implemented |
| **Rainbow / Hue Cycle** | Continuous | Smooth HSV cycle over a duration. Software-driven repeated single-colour commands stepping through hue at 15-30 updates/sec. | cycle speed (Hz), brightness, duration, target group | ✅ Implemented |
| **Starlight** | Continuous (irregular) | Sparse, randomly-timed twinkles using cool/warm white palette and low chance value. Long release for fading-star quality. Designed for ambient/contemplative passages. | palette, mean interval, jitter, ASR, chance, target group | ✅ Implemented |
| **Wave** | Per-beat | Sequential firing across an ordered group of targets. Bracelet 1 fires on beat, bracelet 2 fires 100ms later, etc. Creates a ripple across the constellation. | colour, ASR, inter-target delay, ordering, target group | ⏳ Designed, not implemented |
| **Gradient Hold** | One-shot or continuous | Different fixed colour per target across an ordered group. Each target stays at its assigned colour with optional slow breathing. Used for the lunar/constellation gradient where each bracelet is a different shade. | per-target colours, sustain, optional breathe period, target group | ⏳ Designed, not implemented |
| **Strobe Burst** | One-shot | Rapid succession of short-envelope pulses (typically 4-8 fires over 500-800ms). Used for drops or dramatic moments. | colour, burst count, inter-pulse gap, target group | ⏳ Designed, not implemented |
| **Background Wash** | Continuous | Low-intensity sustained colour that holds between other effects. Provides a non-dark idle state. PixMob bracelets that support background colour write it directly to EEPROM; others simulate via long-sustain repeated commands. | colour, brightness | ⏳ Designed, model-dependent |
| **Two-Colour Flash** | Per-beat | Brief flash of colour A followed by sustained colour B. PixMob's native CommandTwoColors. Useful for kick-and-tail effects in one IR send. | flash colour, sustain colour, sustain duration, target group | ⏳ Designed, model-dependent |
...
| Musical event | Default effect |
|---|---|
| Kick (bass-band onset) | Pulse on "primary" group |
| Snare (mid-band onset, future) | Pulse on "secondary" group, contrasting colour |
| Sustained quiet section | Starlight + Background Wash |
| Build-up (rising flux density, future) | Hue Cycle accelerating, brightness rising |
| Drop (MUSIC_EVENT 0x06 event_type 1; bass-energy ratio crossing) | Strobe Burst + palette switch. Producer ships in Epic 4.5; consumer-side effect binding is Epic 4.7 territory. |
| Breakdown (MUSIC_EVENT 0x06 event_type 2; sustained low-energy section) | Fade to dim ambient palette. Same producer/consumer split as Drop. |
| Chorus (sustained higher energy, future) | Random Palette Pulse with denser palette |
| Verse (lower energy) | Pulse on single colour, lower brightness |
| Outro / fadeout | Gradient Hold with slow breathe, fading to black |
...
| Platform | Display | Resolution | Capabilities |
|---|---|---|---|
| **M5StickC Plus2** | 1.14" colour TFT | 135 × 240 px | 16-bit colour, text, basic graphics primitives, sprites, font rendering. Backlight controllable. |
| **EMF Tildagon badge** | Round colour LCD | 240 × 240 px (circular) | Full colour, MicroPython framebuffer, SDK font and icon helpers. Plus twelve perimeter RGB LEDs as a separate "ring" surface. |
| **M5Stack Atom Lite** | None | — | Single onboard RGB LED only. No screen. |
| **M5Stack Core2** *(future)* | 2.0" touchscreen | 320 × 240 px | Touch input, full colour, suitable for a portable operator console. |
| **Mac/PC** | Laptop display | — | QLC+ console UI, custom Python visualisations via TouchDesigner or similar. |
...
A Lume's display is **also a light surface in the show**. When a Stick is in Lume mode, an inbound `LIGHT_PULSE` (or any locally-fired effect) paints the screen full-bleed with the broadcast colour and the matching attack/sustain/release fade - the Stick on a tripod becomes a coherent piece of installation gear, not just a transmitter that drives lights but one of the lights itself. For the constellation art piece a single Stick can act as both Director AND visible light point, reducing the bracelet count needed. Outside Lume mode the LCD reverts to its **UI role** (menus, status, calibration prompts in Director / Test / Config modes); the wash family is not painted to the local LCD on the broadcasting Stick by design — there is no Director-side wash loopback, and visual verification of wash requires a second device acting as Lume.
...
**Slave-side: ****`OutputBinding`**** plug-ins.** *(Note: "Slave-side" reflects code identifiers; the operational role is the Lume side.)* An output binding consumes `RenderEvent`s and turns them into hardware action. The contract lives in [`include/output_bindings/output_binding.h`](https://github.com/ratcliffej/nocturnation-stickc/blob/main/include/output_bindings/output_binding.h): same Plugin base as Visualisation, same property-bag and capability machinery. Differences from Visualisation: bindings have no `render_fx` accessor (they *are* the render destination, calling `render_fx` from one would be circular) and their hook surface is `on_light_pulse` rather than `on_audio_frame` (renamed from `on_light_command` in Epic 6C Phase C; the wash family adds parallel `on_light_wash` / `on_light_wash_end` / `on_light_wash_pulse` hooks). `SlaveMode` is a thin shell that fans inbound `LIGHT_PULSE` events out to every registered binding.
...
`render_fx` targets become `"\<class\>:\<group\>"` in hex: `"00:00"` = everyone-everywhere, `"01:00"` = every light-class device any group, `"01:07"` = light-class group 7, `"03:01"` = Tildagon-class group 1. The Director encodes both bytes into `LIGHT_PULSE.target_class` and `target_group` (§4.3); Lumes filter inbound frames against `(binding.device_class(), binding.configured_group())` per active binding and only fan out where both axes match. Relay bindings (`PixMobIrBinding`) bypass the Lume-side group filter on inbound LIGHT_PULSE - the bracelet protocol does its own group filtering at the IR layer.
...
**Per-binding capability surface (Epic 6C).** Each `OutputBinding` also declares a `BindingCapabilities` struct via the pure-virtual `capabilities()` method on the base class:

```cpp
struct BindingCapabilities {
    bool can_pulse;     // Fire an ASR pulse (universal; every binding sets this true).
    bool can_wash;      // Hold a persistent background wash (LIGHT_WASH family).
    bool can_overlay;   // Render a PULSE additively on top of an active WASH.
    // Future: can_strobe, can_text, can_animate_screen, ...
};
```

The struct is extensible — future flags slot in without breaking existing bindings (they default-init to `false`). Current declarations:

| Binding | `device_class()` | `can_pulse` | `can_wash` | `can_overlay` |
|---|---|:---:|:---:|:---:|
| `PixMobIrBinding`              | `Light`          | ✓ | — | — |
| `LocalDisplayBinding` (StickC) | `Screen`         | ✓ | ✓ | ✓ |
| Tildagon perimeter + LCD       | `MultiLedScreen` | ✓ | ✓ | ✓ |

**Capability filtering happens at the dispatch layer**, not inside the bindings. `LumeMode::fan_out_light_wash[_end|_pulse]` short-circuits per-binding routing with `if (!slot.binding->capabilities().can_wash) continue;` before invoking the `on_light_*` hook. Wire frames addressed to PixMob-class targets still pass the existing `(target_class, target_group)` filter (since PixMobIrBinding declares `device_class() == Light`), but the capability gate drops them before `on_light_wash` would be called. PixMobIrBinding therefore needs no wash-aware code; the dispatch enforces its pulse-only behaviour by construction. The full receive-side filter is documented in [lume-capabilities-design.md](lume-capabilities-design.md) §5 + the Phase B audit.

Why pure-virtual rather than a default-true struct: a silent default of `{true, true, true}` would let a new binding inherit "can wash" by accident, the dispatch would then route washes to it that it cannot render, and operators would see lost effects with no log to point at. Forcing every binding to declare explicitly is one extra line per subclass; the alternative was paid in subtle bugs.

Future hosts add their own bindings without touching Lume-mode code: Tildagon (Epic 5) shipped a perimeter LED ring + LCD pair (now wash-capable per Epic 6C Phase G); a DMX deployment (Epic 7) lands a `DmxOutputBinding`; a BLE-controlled bracelet line lands a `BleBinding`. None of those require a recompile of `SlaveMode`.
...
| Mode | Behaviour | Required hardware |
|---|---|---|
| **Autonomous Director** | Run local audio analysis, fire local outputs (IR/LED/screen), broadcast beat events on ESP-NOW for any listeners. Continues running indefinitely whether or not audio is present - silence is treated as a valid state, not a failure. Optional **Audio-silence failover** config (default OFF; see §8.4) demotes the node to Lume Mode after a configurable silence period, intended for unattended deployments only. | A beat source (mic analyser **or** IMU tap-to-beat) + at least one output. Most hosts use the mic analyser; the Tildagon has **no microphone** but runs an **IMU tap-to-beat Director** (Epic 6B, §8.9). The mode-selection menu offers Director on any host with a beat source. |
| **Lume** | Listen on configured ESP-NOW channel for events; fire local outputs in response. Optionally rebroadcasts received frames if the **Repeat** config option is enabled (off by default; see §8.4). Falls back to a local idle effect (subtle hue cycle, starlight, etc.) after N seconds without Director heartbeat. | ESP-NOW + at least one output |
| **Config** | Configuration menus; outputs muted while in this mode. | Any UI |
| **Test** | Manual triggering of effect primitives for hardware validation and play; mic and ESP-NOW listening suspended. | Any UI + at least one output |
| **Idle / Off** | All outputs muted; ESP-NOW listening suspended. Display shows clock or logo. Long-press of power button enters this mode from anywhere. | None |
...
| Platform | Input | Display surface | Notes |
|---|---|---|---|
| **M5StickC Plus2** | Btn A (select), Btn B (cycle), Btn PWR (back/power) | 1.14" TFT, \~5 lines of text at size 2 | Three buttons sufficient for menu navigation; long-press PWR for idle/off. |
| **EMF Tildagon** | 6 perimeter buttons | 240×240 round LCD | Richer UI possible; could use radial menu following the badge's circular form. |
| **M5Stack Atom Lite** | 1 button | 1 RGB LED | Limited UI; uses LED colour to indicate mode (red=Director, blue=Lume, white=Test). Long-press to cycle modes; single-press for default action within mode. Config done via serial console or one-shot "setup mode" via button held during boot. |
...
### 12.3 Lighting design references (§1.2)
Chauvet Professional (2024) *Felix Peralta - Lyrical Light* \[Interview\]. Available at: [https://www.chauvetprofessional.com/news/felix-peralta-lyrical-light/](https://www.chauvetprofessional.com/news/felix-peralta-lyrical-light/) (Accessed: 22 May 2026). Interview with Emmy-winning concert lighting designer Felix Peralta covering the musicality-led approach to concert lighting design, the role of the LD's musical background, and the principle of reflecting "timing, pacing, and tempo" rather than surface beat. Cited in §1.2 for the bass-as-lead and song-arc-first design principles.
Fisher, R.S., Acharya, J.N., Baumer, F.M., French, J.A., Parisi, P., Solodar, J.H., Szaflarski, J.P., Thio, L.L., Tolchin, B., Wilkins, A.J. and Kasteleijn-Nolst Trenité, D. (2022) 'Visually sensitive seizures: An updated review by the Epilepsy Foundation', *Epilepsia*, 63(4), pp. 739-768. Available at: [https://pubmed.ncbi.nlm.nih.gov/35132632/](https://pubmed.ncbi.nlm.nih.gov/35132632/) (Accessed: 22 May 2026). Updated review of the photosensitive seizure literature for the Epilepsy Foundation of America, covering provocative flash rates (3-60 Hz, peak 15-25 Hz), brightness thresholds, and field-of-view coverage. Cited in §1.2 (safety floor) and complementing §15 (Safety considerations).
Lose, C. (2019-present) *LD-at-Large Podcast* \[Audio podcast\]. Available at: [https://open.spotify.com/show/0tbmoM7NsGquLMMgQqNEUX](https://open.spotify.com/show/0tbmoM7NsGquLMMgQqNEUX) (Accessed: 22 May 2026). Working concert lighting designers interviewed by Chris Lose (Designer Relations Developer at Ayrton Lighting and international touring LD). Topics include the "less-is-more principle" (Music-by-Sia episode), lighting as function vs. art, training the next generation, and how musical background informs LD craft. Cited in §1.2 for the restraint-earns-impact design principle.
Pilbrow, R. (2017) *Performance Lighting Design: How to Light for the Stage, Concerts and Live Events*. London: Methuen Drama / Bloomsbury Publishing. ISBN 978-1-3500-1707-8. Standard reference textbook for performance lighting design covering cue structure, lighting score notation, the LD-as-musician framing, and the practical and artistic separation between functional and expressive lighting. Cited in §1.2 as a longer-form resource for Show authors developing their lighting design instincts.
SHEHDS (2025) *Concert Stage Lighting: How to Set Up Concert Stage Lights?* \[Online guide\]. Available at: [https://shehds.com/blogs/news/concert-stage-lighting](https://shehds.com/blogs/news/concert-stage-lighting) (Accessed: 22 May 2026). Operator-level guide aimed at small-venue lighting work; relevant for its emphasis on building base lighting looks per song section, the principle that "not everything needs equal brightness simultaneously," and the genre-specific pacing guidance. Cited in §1.2 for the layered-timescales design principle.
SHEHDS (2026) *Rock Concert Lighting Design: How to Create Powerful Performances* \[Online guide\]. Available at: [https://shehds.com/blogs/news/rock-concert-lighting-design](https://shehds.com/blogs/news/rock-concert-lighting-design) (Accessed: 22 May 2026). Companion guide focused on rock-genre lighting practice, with discussion of timing lighting cues to musical changes and the use of contrast over comfort. Referenced alongside the 2025 general guide in §1.2.
...
| Platform | Battery | Typical Nocturnation runtime | Notes |
|---|---|---|---|
| **M5StickC Plus2** | 200 mAh internal Li-ion | \~2 hours | FFT and IR transmission are the dominant load. Plug in via USB-C for indefinite operation; small power bank doubles runtime trivially. |
| **PixMob bracelet** (Aurora) | 2 × AAA | Tens of hours of intermittent use; original 2024 Coldplay tour batteries still functional in May 2026 | Bracelets sleep between IR commands; only LED illumination during ASR envelopes draws meaningful current. Not a near-term concern. |
| **PixMob bracelet** (older CR1632) | 2 × CR1632 coin cell | Several hours of active use | Lower capacity than AAA models; replacement coin cells inexpensive and easy to source. |
| **EMF Tildagon** | Shared with badge functions (multi-day at idle) | \~6-8 hours under Nocturnation receive workload (estimate) | ESP-NOW listening + screen + LEDs noticeably faster than idle drain. Calm mode and aggressive sleep when no broadcasts heard for \>30s recommended. |
| **M5Stack Atom Lite** | None (USB-powered) | Indefinite (mains) | No internal battery. Suitable only for fixed installations or wired deployments. |
...
- **v0.31** (2026-05-22): added **§1.2 "Lighting design principles"** as a normative chapter for Show authors. Captures the working-LD insight (prompted by a gig observation) that flashing on every beat looks amateurish, exhausts audiences, and undermines the music - and codifies the project's design aesthetic accordingly. Six framed principles: (1) three layered timescales (song arc / phrase / beat) with the beat as punctuation not foundation; (2) bass as lead, not hi-hat - the analyser's existing sub-band split already serves this; (3) restraint earns the moments that matter (drops only land if the verse was quieter); (4) the medical safety floor (3-60 Hz photosensitive seizure range) sits well below where well-designed shows operate aesthetically; (5) implications for Show authors as a concrete checklist; (6) implications for Tildagon Director Mode (Epic 6 Workstream B) and the DMX direction (Epic 7) - NocturNation is a *fixture*, not a lighting designer, and in DMX mode the aesthetic decisions move into the LD's cue stack where they belong. New §12.3 references section captures the six grounding sources: Chauvet's Felix Peralta interview, Fisher et al. 2022 Epilepsia review, Lose's LD-at-Large podcast, Pilbrow's Performance Lighting Design textbook, and two SHEHDS operator guides. All references verified live 2026-05-22. The chapter is positioned as design *philosophy* rather than implementation: the protocol stays permissive, but Shows that aspire to look good follow this guidance. Adding it strengthens Epic 6's launch narrative by visibly demonstrating that NocturNation understands the craft, which matters for EMF community credibility and any future professional partnerships.
- **v0.32** (2026-05-31): **Epic 6C close - Lume capabilities + LIGHT_PULSE rename + LIGHT_WASH family + ADR crossfade fix.** Headline protocol additions and surface-level changes that make the §1.2 "continuous wash baseline + sparing punctuation" aesthetic implementable rather than aspirational. §4.3 message-types table grows from two active types to five: `LIGHT_COMMAND` renamed to `LIGHT_PULSE` (wire byte 0x03 unchanged, payload unchanged - rename frees the "LIGHT_" prefix for the wash family); three new types land in the v0.27-deprecated slots - `LIGHT_WASH` (0x06, 16-byte payload carrying two RGB triplets + cycle_ms + intensity + TTL + pulse_response for cosine-eased ping-pong drift between two colours), `LIGHT_WASH_END` (0x07, 3-byte explicit cancel with operator release_time), `LIGHT_WASH_PULSE` (0x08, 9-byte pulse that fires only on currently-washing Lumes). §4.3 "Why two messages, not one" rewritten as "Why five messages, not two" with per-message rationale for each new type. §7.6 plug-in surfaces gains the **`BindingCapabilities`** struct (`can_pulse` / `can_wash` / `can_overlay`) with per-binding declarations - dispatch capability-gates the wash family so PixMob bindings silently drop wash frames without per-binding boilerplate. §1.2 gains a new **"Implementation differs by Lume capability"** subsection making the §1.2 aesthetic achievable on both capable Lumes (LIGHT_WASH baseline) and pulse-only PixMob-class fleets (Show-driven low-cadence PULSE re-broadcasts). Plus a render-side fix in the firmware: the perimeter / LCD pulse renderer now lerps from the LED's *current rendered colour* during the attack phase, so a rainbow palette with attack > 0 crossfades smoothly between hues instead of snapping through black (`T_0_MS` attack still snaps - explicit jump-cut semantics). The wire surface gains five bytes net (the 16-byte LIGHT_WASH payload is the only new traffic at scale; LIGHT_WASH is a Director-side state transition, not a periodic flash, so it doesn't load the channel). No protocol-version bump because the project isn't deployed externally and the v0.27-deprecated 0x06/0x07/0x08 slots had no live parsers to break. The companion design doc, [lume-capabilities-design.md](lume-capabilities-design.md), holds the full renderer contract (state machine, cosine formula, supersede semantics, single-Stick demo caveat) plus the Phase B / Phase F implementation audits. The Tildagon firmware (nocturnation-tildagon) mirrors the same surface (Phase G). Post-PR polish pass also (a) made the LCD-role-per-mode rule explicit in §7.4 — the StickC LCD is UI in Director / Test / Config modes and a lighting surface only in Lume mode, so there is no Director-side wash loopback (closed-NO; visual wash verification requires a second device acting as Lume), (b) moved the operator-facing Wash Test entry from `Config > Utilities` to `Test Mode` alongside the rest of the §8.5 hardware-verification catalogue (`Utilities` is for developer bench tools), and (c) swept the remaining present-tense `LIGHT_COMMAND` references in §1.1 / §2 / §4.4 / §6 / §7.4 / §7.6 to `LIGHT_PULSE` (Phase C oversight; historical version-history entries deliberately preserve the older name).
